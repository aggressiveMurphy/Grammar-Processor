...
{//构造函数：建立log日志文件以便记录和展示处理过程；读取文法规则并建立文法结点，连接成grammar
    //先创建日志文件
    log.open("log.txt");
    if(!log.is_open())
    {
...
void Processor::printGrammars()
{//将当前信息输入到日志文件中
    for(size_t i = 0; i < grammar.size(); i++)
    {
        const string &left = grammar[i].left;
        const vector<vector<string>> &right = grammar[i].right;
...
{//消除左递归
    ...
    for(size_t i = 0; i < grammar.size(); i++)
    {
        for(size_t j = 0; j < i; j++)
        {
            ...
            vector< vector<string> > &right1 = grammar[i].right;
            vector< vector<string> > &right2 = grammar[j].right;
            string check = grammar[j].left;
    ...
    int sizeAfter = grammar.size();
    if(sizeAfter > sizeBefore)
        startWordIndex = grammar.size()-1;
    else
        startWordIndex = 0;
    ...
    {
        //左边部分
        string check = grammar[i].left;
        //右边部分
        vector<vector<string>> temp = grammar[i].right;
        ...
        vector<vector<string>> cont;
        //指向最后一条文法结点
        vector<vector<string>> &ss = grammar[grammar.size()-1].right;
        vector<string> eplision;
        eplision.push_back("@");
        //ss: {{"@"}}
        ss.push_back(eplision);
        vector<vector<string>>&temp1 = grammar[i].right;
        while(!temp1.empty())
        {
            //对于A->Aa|b，如果现在处理的是Aa
...
{//求非终结符号word是出现在第几条文法规则的左边
    for(size_t i = 0; i < grammar.size(); i++)
    {
        if(grammar[i].left == word)
            return i;
    }
    return -1;
}
...
{//求第i条规则左边的非终结符号的first集合
    ...
    visited[i] = true;
    //规则左边
    string left = grammar[i].left;
    //规则右边
    vector<vector<string>> right = grammar[i].right;
...
{//删除无用的文法规则,有害的文法规则（形如A->A）已经在读取文件的时候去除
...
{//处理左公因子
    for(size_t i = 0; i < grammar.size();)
    {
        ...
        vector<LinkNode> afterDeal = dealCommonFactor(grammar[i]);
        if(afterDeal.empty())
        {
        ...
        grammar.insert(grammar.end(), afterDeal.begin(), afterDeal.end());
    }
    //找到开始符号
    for(size_t i = 0;i<grammar.size();i++)
    {
        if(grammar[i].left == startNode)
        {
            startWordIndex = i;
            break;
        }
...
{//处理单条文法的左公因子,每次只提取一位的公因子，执行多次即可提取多位
    //返回的结果
    //例如 A->ab|ac|bd
    //则返回A->aA'|bd和A'->b|c的向量
...
string Processor::generateNewName(string source)
{
    ...
    while(true)
    {
        for(size_t uu = 0; uu < grammar.size(); uu++)
        {
            if(str == grammar[uu].left)
            {
                str += "'";
                break;
            }
...
{//求Follow集合
    follow[grammar[startWordIndex].left].insert("$");
...
vector<vector<string>> Processor::leftMostDerivation()
{
    ...
    string cur = grammar[startWordIndex].left;
    vector<string> v;
    v.push_back(cur);
    output.push_back(v);
    //表示目前已经对应到目标字符串的第几个
    ...
    while(true)
    {
        ...
        for(vector<string> temp : grammar[getIndex(word)].right)
        {
            if(temp[0] == EPSILON)
            {
                //follow集合包含
